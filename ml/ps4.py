...
def r_squared(y, estimated):
    ...
    variance = ((np.array(y) - np.mean(y))**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        pylab.title('Estimate models')
        pylab.xlabel('x')
        pylab.ylabel('y')
        pylab.plot(x, y, 'bo', label = 'Data')
        estY = np.polyval(model, x)
        pylab.plot(x, estY, 'r',
                label = 'Model ' + str(len(model)) + "-polynom" + ', R2 = '
                + str(round(r_squared(y, estY), 4)))
...
# models = generate_models(x, y, [7, 8, 9])
