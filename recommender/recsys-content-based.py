...
# We will use the One Hot Encoding technique to convert the list of genres
# to a vector where each column corresponds to one possible value of the feature.
...
# With the input complete, let's extract the input movie's ID's
# from the movies dataframe and add them into it.

# We can achieve this by first filtering out the rows that contain
# the input movie's title and then merging this subset with the input dataframe.
...
# into the input's genre table and then summing up the resulting table by column.
...
# This is known as the User Profile. Using this, we can recommend movies
