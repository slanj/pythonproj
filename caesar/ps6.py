...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        i = 0
        for letter in lower:
            shift_dict[letter] = lower[(i+shift) % 26]
            i += 1
        j = 0
        for letter in upper:
            shift_dict[letter] = upper[(j+shift) % 26]
            j += 1
    ...
    def apply_shift(self, shift):
        ...
        '''
        txt = self.get_message_text()
        encrypted = []
        shift_dict = self.build_shift_dict(shift)
        for letter in txt:
            if letter in shift_dict.keys():
                encrypted.append(shift_dict[letter])
            else:
